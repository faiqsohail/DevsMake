// Code generated by go-swagger; DO NOT EDIT.

package auth

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the generate command

import (
	"context"
	"net/http"

	"github.com/go-openapi/runtime/middleware"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// GetAuthLoginHandlerFunc turns a function with the right signature into a get auth login handler
type GetAuthLoginHandlerFunc func(GetAuthLoginParams) middleware.Responder

// Handle executing the request and returning a response
func (fn GetAuthLoginHandlerFunc) Handle(params GetAuthLoginParams) middleware.Responder {
	return fn(params)
}

// GetAuthLoginHandler interface for that can handle valid get auth login params
type GetAuthLoginHandler interface {
	Handle(GetAuthLoginParams) middleware.Responder
}

// NewGetAuthLogin creates a new http.Handler for the get auth login operation
func NewGetAuthLogin(ctx *middleware.Context, handler GetAuthLoginHandler) *GetAuthLogin {
	return &GetAuthLogin{Context: ctx, Handler: handler}
}

/* GetAuthLogin swagger:route GET /auth/login auth getAuthLogin

login through github oauth2

*/
type GetAuthLogin struct {
	Context *middleware.Context
	Handler GetAuthLoginHandler
}

func (o *GetAuthLogin) ServeHTTP(rw http.ResponseWriter, r *http.Request) {
	route, rCtx, _ := o.Context.RouteInfo(r)
	if rCtx != nil {
		*r = *rCtx
	}
	var Params = NewGetAuthLoginParams()
	if err := o.Context.BindValidRequest(r, route, &Params); err != nil { // bind params
		o.Context.Respond(rw, r, route.Produces, route, err)
		return
	}

	res := o.Handler.Handle(Params) // actually handle the request
	o.Context.Respond(rw, r, route.Produces, route, res)

}

// GetAuthLoginOKBody get auth login o k body
//
// swagger:model GetAuthLoginOKBody
type GetAuthLoginOKBody struct {

	// access token
	AccessToken string `json:"access_token,omitempty"`
}

// Validate validates this get auth login o k body
func (o *GetAuthLoginOKBody) Validate(formats strfmt.Registry) error {
	return nil
}

// ContextValidate validates this get auth login o k body based on context it is used
func (o *GetAuthLoginOKBody) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (o *GetAuthLoginOKBody) MarshalBinary() ([]byte, error) {
	if o == nil {
		return nil, nil
	}
	return swag.WriteJSON(o)
}

// UnmarshalBinary interface implementation
func (o *GetAuthLoginOKBody) UnmarshalBinary(b []byte) error {
	var res GetAuthLoginOKBody
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*o = res
	return nil
}
